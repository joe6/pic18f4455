//===-- PIC18F4455Overlay.cpp - Implementation for PIC18F4455 Frame Overlay===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the PIC18F4455 Frame Overlay implementation.
//
//===----------------------------------------------------------------------===//
...
namespace llvm {
  char PIC18F4455Overlay::ID = 0;
  ModulePass *createPIC18F4455OverlayPass() { return new PIC18F4455Overlay(); }
}

void PIC18F4455Overlay::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
  AU.addRequired<CallGraph>();
}

void PIC18F4455Overlay::DFSTraverse(CallGraphNode *CGN, unsigned Depth) {
  ...
  if (Depth != 0 && CGN->getFunction()) {
    ...
    if (Color >= PIC18F4455OVERLAY::StartIndirectCallColor || 
        Depth >= PIC18F4455OVERLAY::StartIndirectCallColor) {
...
unsigned PIC18F4455Overlay::ModifyDepthForInterrupt(CallGraphNode *CGN,
                                                    unsigned Depth) {
...
void PIC18F4455Overlay::setColor(Function *Fn, unsigned Color) {
  std::string Section = "";
  if (Fn->hasSection())
...
unsigned PIC18F4455Overlay::getColor(Function *Fn) {
  int Color = 0;
  if (!Fn->hasSection())
    return 0;
...
bool PIC18F4455Overlay::runOnModule(Module &M) {
...
void PIC18F4455Overlay::MarkIndirectlyCalledFunctions(Module &M) {

//===-- PIC18F4455AsmPrinter.cpp - PIC18F4455 LLVM assembly writer ------------------===//
//
...
#include "PIC18F4455AsmPrinter.h"
#include "PIC18F4455Section.h"
...
#include "PIC18F4455GenAsmWriter.inc"

PIC18F4455AsmPrinter::PIC18F4455AsmPrinter(TargetMachine &TM, MCStreamer &Streamer)
: AsmPrinter(TM, Streamer), DbgInfo(Streamer, TM.getMCAsmInfo()) {
  PMAI = static_cast<const PIC18F4455MCAsmInfo*>(TM.getMCAsmInfo());
  //PTOF = &getObjFileLowering();
}

void PIC18F4455AsmPrinter::EmitInstruction(const MachineInstr *MI) {
  SmallString<128> Str;
...
void PIC18F4455AsmPrinter::ColorAutoSection(const Function *F) {
  std::string SectionName = PAN::getAutosSectionName(CurrentFnSym->getName());
  //PIC18F4455Section* Section = PTOF->findPIC18F4455Section(SectionName);
  /*
  if (Section != NULL) {
...
///
bool PIC18F4455AsmPrinter::runOnMachineFunction(MachineFunction &MF) {
...
void PIC18F4455AsmPrinter::printOperand(const MachineInstr *MI, int opNum,
                                   raw_ostream &O) {
    ...
    case MachineOperand::MO_Register:
      {
        ...
        if ((MI->getOpcode() == PIC18F4455::load_indirect) ||
            (MI->getOpcode() == PIC18F4455::store_indirect))
...
///
void PIC18F4455AsmPrinter::printCCOperand(const MachineInstr *MI, int opNum,
                                     raw_ostream &O) {
  int CC = (int)MI->getOperand(opNum).getImm();
  //O << PIC18F4455CondCodeToString((PIC18F4455CC::CondCodes)CC);
}

/// printLibcallDecls - print the extern declarations for compiler 
/// intrinsics.
///
void PIC18F4455AsmPrinter::printLibcallDecls() {
...
///
bool PIC18F4455AsmPrinter::doInitialization(Module &M) {
  ...
  for (Module::global_iterator I = M.global_begin(), E = M.global_end();
    ...
    if (!I->hasAvailableExternallyLinkage()) {
      ...
      I->setSection(((const PIC18F4455Section *)S)->getName());
    }
  }
...
///
void PIC18F4455AsmPrinter::EmitFunctionDecls(Module &M) {
...
void PIC18F4455AsmPrinter::EmitUndefinedVars(Module &M) {
...
void PIC18F4455AsmPrinter::EmitDefinedVars(Module &M) {
...
void PIC18F4455AsmPrinter::EmitRomData(Module &M) {
  //EmitSingleSection(PTOF->ROMDATASection());
}

// Emit Shared section udata.
void PIC18F4455AsmPrinter::EmitSharedUdata(Module &M) {
  //EmitSingleSection(PTOF->SHAREDUDATASection());
}

bool PIC18F4455AsmPrinter::doFinalization(Module &M) {
  EmitAllAutos(M);
  printLibcallDecls();
  DbgInfo.EndModule(M);
...
void PIC18F4455AsmPrinter::EmitFunctionFrame(MachineFunction &MF) {
  const Function *F = MF.getFunction();
...
/*  
  PIC18F4455Section *fPDataSection =
    const_cast<PIC18F4455Section *>(getObjFileLowering().
...
/*
void PIC18F4455AsmPrinter::EmitInitializedDataSection(const PIC18F4455Section *S) {
  /// Emit Section header.
...
void PIC18F4455AsmPrinter::EmitIData(Module &M) {
  //EmitSectionList (M, PTOF->IDATASections());
}
/*
void PIC18F4455AsmPrinter::
EmitUninitializedDataSection(const PIC18F4455Section *S) {
    const TargetData *TD = TM.getTargetData();
    OutStreamer.SwitchSection(S);
    std::vector<const GlobalVariable*> Items = S->Items;
    for (unsigned j = 0; j < Items.size(); j++) {
...
void PIC18F4455AsmPrinter::EmitUData(Module &M) {
  //EmitSectionList (M, PTOF->UDATASections());
}

// Print all USER sections.
void PIC18F4455AsmPrinter::EmitUserSections(Module &M) {
  //EmitSectionList (M, PTOF->USERSections());
}

// Print all AUTO sections.
void PIC18F4455AsmPrinter::EmitAllAutos(Module &M) {
  //EmitSectionList (M, PTOF->AUTOSections());
}

extern "C" void LLVMInitializePIC18F4455AsmPrinter() { 
  RegisterAsmPrinter<PIC18F4455AsmPrinter> X(ThePIC18F4455Target);
}
/*
// Emit one data section using correct section emitter based on section type.
void PIC18F4455AsmPrinter::EmitSingleSection(const PIC18F4455Section *S) {
  if (S == NULL) return;
...
/*
void PIC18F4455AsmPrinter::
EmitSectionList(Module &M, const std::vector<PIC18F4455Section *> &SList) {

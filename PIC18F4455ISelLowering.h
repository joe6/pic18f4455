//===-- PIC18F4455ISelLowering.h - PIC18F4455 DAG Lowering Interface ------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that PIC18F4455 uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//

#ifndef PIC18F4455ISELLOWERING_H
#define PIC18F4455ISELLOWERING_H
...
namespace llvm {
  namespace PIC18F4455ISD {
    enum NodeType {
      ...
      PIC18F4455Load,
      PIC18F4455LdArg,   // This is replica of PIC18F4455Load but used to load function 
      ...
      PIC18F4455Connect,  // General connector for PIC18F4455 nodes
      BCF,
      LSLF,          // PIC18F4455 Logical shift left
      LRLF,          // PIC18F4455 Logical shift right
      RLF,           // Rotate left through carry
      RRF,           // Rotate right through carry
      CALL,          // PIC18F4455 Call instruction 
      CALLW,         // PIC18F4455 CALLW instruction 
    ...
    enum PIC18F4455Libcall {
      MUL_I8 = RTLIB::UNKNOWN_LIBCALL + 1,
      SRA_I8,
      SLL_I8,
      SRL_I8,
      PIC18F4455UnknownCall
    };
  }
  ...
  //===--------------------------------------------------------------------===//
  class PIC18F4455TargetLowering : public TargetLowering {
  public:
    explicit PIC18F4455TargetLowering(PIC18F4455TargetMachine &TM);
    ...
    SDValue getPIC18F4455Cmp(SDValue LHS, SDValue RHS, unsigned OrigCC, SDValue &CC,
...
//                                     SelectionDAG &DAG) const;
   ...
   virtual SDValue
    ...
    SDValue PerformPIC18F4455LoadCombine(SDNode *N, DAGCombinerInfo &DCI) const; 
  ...
  private:
    ...
    // Load pointer can be a direct or indirect address. In PIC18F4455 direct
    ...
    /// Subtarget - Keep a pointer to the PIC18F4455Subtarget around so that we can
    /// make the right decision when generating code for different targets.
    const PIC18F4455Subtarget *Subtarget;


    // Extending the LIB Call framework of LLVM
    // to hold the names of PIC18F4455Libcalls.
    const char *PIC18F4455LibcallNames[PIC18F4455ISD::PIC18F4455UnknownCall]; 

    // To set and retrieve the lib call names.
    void setPIC18F4455LibcallName(PIC18F4455ISD::PIC18F4455Libcall Call, const char *Name);
    const char *getPIC18F4455LibcallName(PIC18F4455ISD::PIC18F4455Libcall Call) const;

    // Make PIC18F4455 Libcall.
    SDValue MakePIC18F4455Libcall(PIC18F4455ISD::PIC18F4455Libcall Call, EVT RetVT, 
...
#endif // PIC18F4455ISELLOWERING_H

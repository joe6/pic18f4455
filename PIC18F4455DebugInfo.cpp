...
//===-- PIC18F4455DebugInfo.cpp - Implementation for PIC18F4455 Debug Information ======//
//
...
///
void PIC18F4455DbgInfo::PopulateDebugInfo (DIType Ty, unsigned short &TypeNo,
                                      bool &HasAux, int Aux[], 
                                      std::string &TagName) {
  if (Ty.isBasicType())
...
///
void PIC18F4455DbgInfo::PopulateBasicTypeInfo (DIType Ty, unsigned short &TypeNo) {
  std::string Name = Ty.getName();
  unsigned short BaseTy = GetTypeDebugNumber(Name);
  TypeNo = TypeNo << PIC18F4455Dbg::S_BASIC;
...
///
void PIC18F4455DbgInfo::PopulateDerivedTypeInfo (DIType Ty, unsigned short &TypeNo,
                                            bool &HasAux, int Aux[],
                                            std::string &TagName) {
  ...
  switch(Ty.getTag())
  {
    case dwarf::DW_TAG_pointer_type:
      TypeNo = TypeNo << PIC18F4455Dbg::S_DERIVED;
      TypeNo = TypeNo | PIC18F4455Dbg::DT_PTR;
      break;
    default:
      TypeNo = TypeNo << PIC18F4455Dbg::S_DERIVED;
  }
...
void PIC18F4455DbgInfo::PopulateArrayTypeInfo (DIType Ty, unsigned short &TypeNo,
                                          bool &HasAux, int Aux[],
                                          std::string &TagName) {
  ...
  for (unsigned i = 0, N = Elements.getNumElements(); i < N; ++i) {
    ...
    if (Element.getTag() == dwarf::DW_TAG_subrange_type) {
      TypeNo = TypeNo << PIC18F4455Dbg::S_DERIVED;
      TypeNo = TypeNo | PIC18F4455Dbg::DT_ARY;
...
/// structure or union.
///
void PIC18F4455DbgInfo::PopulateStructOrUnionTypeInfo (DIType Ty, 
  ...
  if (Ty.getTag() == dwarf::DW_TAG_structure_type)
    TypeNo = TypeNo | PIC18F4455Dbg::T_STRUCT;
  else
    TypeNo = TypeNo | PIC18F4455Dbg::T_UNION;
  TagName = CTy.getName();
  ...
  // PIC18F4455DebugInfo implementation anyways after the MDNodes based 
...
void PIC18F4455DbgInfo::PopulateEnumTypeInfo (DIType Ty, unsigned short &TypeNo) {
  TypeNo = TypeNo << PIC18F4455Dbg::S_BASIC;
  TypeNo = TypeNo | PIC18F4455Dbg::T_ENUM;
}

/// PopulateCompositeTypeInfo - Populate TypeNo, Aux[] and TagName for 
/// composite types from Ty.
///
void PIC18F4455DbgInfo::PopulateCompositeTypeInfo (DIType Ty, unsigned short &TypeNo,
                                              bool &HasAux, int Aux[],
                                              std::string &TagName) {
  switch (Ty.getTag()) {
...
///
unsigned PIC18F4455DbgInfo::GetTypeDebugNumber(std::string &type)  {
  if (type == "char")
    return PIC18F4455Dbg::T_CHAR;
  else if (type == "short")
    return PIC18F4455Dbg::T_SHORT;
  else if (type == "int")
    return PIC18F4455Dbg::T_INT;
  else if (type == "long")
    return PIC18F4455Dbg::T_LONG;
  else if (type == "unsigned char")
    return PIC18F4455Dbg::T_UCHAR;
  else if (type == "unsigned short")
    return PIC18F4455Dbg::T_USHORT;
  else if (type == "unsigned int")
    return PIC18F4455Dbg::T_UINT;
  else if (type == "unsigned long")
    return PIC18F4455Dbg::T_ULONG;
  else
    return 0;
}
...
///
void PIC18F4455DbgInfo::BeginFunction(const MachineFunction &MF) {
...
///
void PIC18F4455DbgInfo::ChangeDebugLoc(const MachineFunction &MF,  
...
///
void PIC18F4455DbgInfo::SwitchToLine(unsigned Line, bool IsInBeginFunction) {
...
///
void PIC18F4455DbgInfo::EndFunction(const MachineFunction &MF) {
  if (! EmitDebugDirectives) return;
  const DebugLoc &DL = GetDebugLocForFunction(MF);
  // Emit debug info only if valid debug info is available.
  if (!DL.isUnknown())
...
///
void PIC18F4455DbgInfo::EndModule(Module &M) {
  if (! EmitDebugDirectives) return;
  EmitVarDebugInfo(M);
...
/// composite type.
/// 
void PIC18F4455DbgInfo::EmitCompositeTypeElements (DICompositeType CTy,
                                              std::string SuffixNo) {
  ...
  for (unsigned i = 0, N = Elements.getNumElements(); i < N; i++) {
    ...
    bool HasAux = false;
    int ElementAux[PIC18F4455Dbg::AuxSize] = { 0 };
    std::string TagName = "";
...
///
void PIC18F4455DbgInfo::EmitCompositeTypeDecls(Module &M) {
  DebugInfoFinder DbgFinder;
  DbgFinder.processModule(M);
  for (DebugInfoFinder::iterator I = DbgFinder.type_begin(),
    ...
    if (CTy.getTag() == dwarf::DW_TAG_union_type ||
      ...
      unsigned short size = CTy.getSizeInBits()/8;
      int Aux[PIC18F4455Dbg::AuxSize] = {0};
      // 7th and 8th byte represent size of structure/union.
      Aux[6] = size & 0xff;
      Aux[7] = size >> 8;
      // Emit .def for structure/union tag.
      if( CTy.getTag() == dwarf::DW_TAG_union_type)
        EmitSymbol(MangledCTyName.c_str(), PIC18F4455Dbg::C_UNTAG);
      else if  (CTy.getTag() == dwarf::DW_TAG_structure_type) 
        EmitSymbol(MangledCTyName.c_str(), PIC18F4455Dbg::C_STRTAG);
      
      // Emit auxiliary debug information for structure/union tag. 
      EmitAuxEntry(MangledCTyName.c_str(), Aux, PIC18F4455Dbg::AuxSize);
      ...
      EmitSymbol(EOSSymbol.c_str(), PIC18F4455Dbg::C_EOS);
      EmitAuxEntry(EOSSymbol.c_str(), Aux, PIC18F4455Dbg::AuxSize, 
...
///
void PIC18F4455DbgInfo::EmitFunctBeginDI(const Function *F) {
  std::string FunctName = F->getName();
  if (EmitDebugDirectives) {
    std::string FunctBeginSym = ".bf." + FunctName;
    std::string BlockBeginSym = ".bb." + FunctName;

    int BFAux[PIC18F4455Dbg::AuxSize] = {0};
    BFAux[4] = CurLine;
    BFAux[5] = CurLine >> 8;

    // Emit debug directives for beginning of function.
    EmitSymbol(FunctBeginSym, PIC18F4455Dbg::C_FCN);
    EmitAuxEntry(FunctBeginSym, BFAux, PIC18F4455Dbg::AuxSize);

    EmitSymbol(BlockBeginSym, PIC18F4455Dbg::C_BLOCK);
    EmitAuxEntry(BlockBeginSym, BFAux, PIC18F4455Dbg::AuxSize);
  }
}

/// EmitFunctEndDI - Emit .ef for function end.
///
void PIC18F4455DbgInfo::EmitFunctEndDI(const Function *F, unsigned Line) {
  std::string FunctName = F->getName();
  if (EmitDebugDirectives) {
    ...
    EmitSymbol(BlockEndSym, PIC18F4455Dbg::C_BLOCK);
    int EFAux[PIC18F4455Dbg::AuxSize] = {0};
    // 5th and 6th byte stand for line number.
    EFAux[4] = CurLine;
    EFAux[5] = CurLine >> 8;
    EmitAuxEntry(BlockEndSym, EFAux, PIC18F4455Dbg::AuxSize);
    EmitSymbol(FunctEndSym, PIC18F4455Dbg::C_FCN);
    EmitAuxEntry(FunctEndSym, EFAux, PIC18F4455Dbg::AuxSize);
  }
}

/// EmitAuxEntry - Emit Auxiliary debug information.
///
void PIC18F4455DbgInfo::EmitAuxEntry(const std::string VarName, int Aux[], int Num,
                                std::string TagName) {
  std::string Tmp;
...
///
void PIC18F4455DbgInfo::EmitSymbol(std::string Name, short Class,
...
///
void PIC18F4455DbgInfo::EmitVarDebugInfo(Module &M) {
  DebugInfoFinder DbgFinder;
  DbgFinder.processModule(M);
  ...
  for (DebugInfoFinder::iterator I = DbgFinder.global_variable_begin(),
    ...
    if (TypeNo != PIC18F4455Dbg::T_NULL) {
      ...
      if (HasAux)
        EmitAuxEntry(VarName, Aux, PIC18F4455Dbg::AuxSize, TagName);
    }
  }
}
...
///
void PIC18F4455DbgInfo::SwitchToCU(MDNode *CU) {
...
///
void PIC18F4455DbgInfo::EmitEOF() {
  if (CurFile != "")
    OS.EmitRawText(StringRef("\t.EOF"));
}
